...
/**
 * Décrit un moteur de jeu de Boogle.
 *
 * @author rouchete
 */
public class Engine {
    ...
    /**
     * Créer un nouveau moteur de jeu de Boogle non initialisé.
     */
    public Engine() {
     ...
     * @throws IOException Problème lors de la lecture du fichier de
     * configuration
     */
     ...
     *
     * @return Joueur dont c'est actuellement le tour, null s'il n'y a pas de
     * tour en cours.
     */
     ...
     * @throws WordTooShortException Le mot saisi est trop court pour être
     * valable.
     * @throws WordNotInDictionaryException Le mot n'est pas dans le
     * dictionnaire.
     * @throws WordNotInLetterGridException Le mot ne fait pas partie de la
     * grille.
    ...
    public void wordInput(String word) throws WordTooShortException,
        ...
        if (getCurrentPlayer().isAlreadyFound(word)) {
            throw new WordAlreadyFoundException(word);
        }
        if (!letterGrid.validWord(word)) {
            throw new WordNotInLetterGridException(word);
        }
    ...
    /**
     * Terminer le tour du joueur et passer au joueur suivant ou terminer la
     * partie.
     */
    public void endTurn() {
        if (!running) {
