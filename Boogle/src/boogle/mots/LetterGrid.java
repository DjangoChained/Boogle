...
public class LetterGrid {
     ...
     * @return True si la grille contient le mot recherché selon les règles du
     * Boggle.
     */
    public boolean validWord(String w) {
        String s = w.toUpperCase();
        if (s.length() < getSize() - 1) {
            return false;
        }
        for (int i = 0; i < getSize(); i++) {
            for (int j = 0; j < getSize(); j++) {
                if (grid[j][i] == s.charAt(0) && findSequence(s, i, j)) {
                    return true;
                }
            }
        }
    ...
    /**
     * Recherche récursive d'une séquence de lettres dans la grille en partant
     * d'une coordonnée.
     *
     * @param s Séquence de lettres à rechercher
     * @param i Coordonnée X de départ
     * @param j Coordonnée Y de départ
     * @return True si la séquence de lettres existe dans la grille et peut
     * former un mot.
     */
    private boolean findSequence(String s, int i, int j) {
        ...
        if (s.length() == 1) {
            return containsLetter(s.charAt(0));
        }
        char c = grid[j][i];
        grid[j][i] = ' ';
        for (int u = -1; u <= 1; u++) {
     ...
     *
     * @param index Index de la lettre dont les voisins devraient contenir la
     * lettre recherchée
     * @param c Lettre à rechercher
     * @return True si les lettres sont côte-à-côte et peuvent être utilisées
     * pour définir un mot valide du Boggle.
     */
